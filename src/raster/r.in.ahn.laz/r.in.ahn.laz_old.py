...
def filter_tiles_by_vector(tiles, vector):
    ...
    try:
        ...
        gs.fatal(
            _("Could not read extent of vector map <{v}>.").format(v=vector)
        )
    ...
    # in `bbox_tiles`, so we can recover the (x, y) after v.select.
    tile_grid = create_temporary_name("ahn_tile_grid")

    # v.in.ascii standard format: one polygon per tile, cat = index+1.
    ascii_lines = ["ORGANIZATION: r.in.ahn.laz", "DIGIT THRESHOLD: 0.0",
                   "MAP NAME: tiles", "MAP DATE: 0", "MAP SCALE: 1",
                   "OTHER INFO: ", "ZONE: 0", "MAP THRESH: 0.0",
                   "VERTI:"]
    ...
    try:
        ...
        gs.fatal(
            _("Failed to build temporary tile grid vector: {err}").format(err=e)
        )

    # Select tiles that overlap with the user vector.
    ...
    for line in cats_out:
        ...
        if not line:
            continue
        # A polygon can have multiple cats printed space-separated; a
        # single tile has one but we stay defensive.
...
def main(options, flags):
    ...
    # with it. This is done after the region-based selection so we still
    # respect the current computational region as the outer bound.
    if vector:
        n_before = len(tiles)
        ...
        gs.message(
            _(
                "Vector filter <{v}>: {k} of {n} candidate tile(s) kept."
            ).format(v=vector, k=len(tiles), n=n_before)
        )
